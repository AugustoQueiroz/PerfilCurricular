...
class Disciplina:
    def __init__(
        self, codigo: str, nome: str, periodo: int, carga_horaria: CargaHoraria
    ):
        self.codigo = codigo
        self.nome = nome
...
class Curso:
    ...
    def perfil_curricular_dot(self):
        ...
        ultimo_periodo = max(
            map(lambda disciplina: disciplina.periodo, self.disciplinas)
        )
        for i in range(1, ultimo_periodo + 2):
            disciplinas = filter(
                lambda disciplina: disciplina.periodo == i - 1, self.disciplinas
            )
            with dot.subgraph() as s:
                ...
                for disciplina in disciplinas:
                    s.node(
                        disciplina.codigo,
                        "%s\n%s" % (disciplina.codigo, disciplina.nome),
                        shape="box",
                        width="3.5",
                    )
                    for prerequisito in disciplina.prerequisitos:
                        dot.edge(
                            disciplina.codigo, prerequisito.codigo, constraint="true"
                        )
